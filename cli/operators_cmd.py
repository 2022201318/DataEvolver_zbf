...
operators_app = typer.Typer(
    ...
    help=t("op.group_help"),
)


_RootOpt = Annotated[
    Optional[Path],
    typer.Option("--root", exists=True, file_okay=False, dir_okay=True, help=t("opt.root_repo")),
]
...
def _print_add_result(out: dict, *, as_json: bool) -> None:
    ...
    if out.get("added"):
        typer.secho(
            tr("已添加算子", "Added operators") + ": " + ", ".join(out["added"]),
    ...
    if isinstance(mem, dict):
        ...
        if promoted_d or promoted_g:
            typer.echo(
                tr("记忆晋升", "Memory promotion")
                + f": domain={promoted_d or '-'}  general={promoted_g or '-'}"
            )
    typer.echo(
        tr("注册表路径", "Registry path")
        + f": {out.get('registry_path')}  ·  "
        + tr("算子池合计", "Pool size")
        + f": {out.get('pool_size')}"
    )
    if out.get("domain_key"):
        typer.echo(tr("域键", "Domain key") + f": {out['domain_key']}")
    typer.secho(
        tr("提示：添加后请重新执行 orchestrate 以在 DAG 中使用新算子。", "Tip: re-run orchestrate to use new operators in the DAG."),
        fg=typer.colors.BLUE,
    )


@operators_app.command("list", help=t("op.list.doc"))
def cmd_list(
    pipeline_id: Annotated[
        str | None,
        typer.Option("--pipeline-id", "-p", help=t("op.list.pipeline")),
    ] = None,
    root: _RootOpt = None,
    source: Annotated[
        str | None,
        typer.Option("--source", help=t("op.list.source")),
    ] = None,
    ...
    if as_json:
        ...
        return
    typer.echo(tr("算子池", "Operator pool") + f"  total={len(ops)}  pipeline={pid or '(global)'}")
...
@operators_app.command("add", help=t("op.add.doc"))
def cmd_add(
    name: Annotated[
        str | None,
        typer.Argument(help=t("op.arg.name")),
    ] = None,
    pipeline_id: Annotated[
        str,
        typer.Option("--pipeline-id", "-p", help=t("op.opt.pipeline_id")),
    ] = "",
    description: Annotated[str, typer.Option("--description", "-d", help=t("op.opt.description"))] = "",
    category: Annotated[
        str,
        typer.Option("--category", "-c", help=t("op.opt.category")),
    ] = "bridge",
    input_keys: Annotated[
        str,
        typer.Option("--input-keys", help=t("op.opt.input_keys")),
    ] = "records",
    output_keys: Annotated[
        str,
        typer.Option("--output-keys", help=t("op.opt.output_keys")),
    ] = "records",
    requires_llm: Annotated[bool, typer.Option("--requires-llm", help=t("op.opt.requires_llm"))] = False,
    from_file: Annotated[
        Path | None,
        typer.Option("--from-file", exists=True, file_okay=True, dir_okay=False, help=t("op.opt.from_file")),
    ] = None,
    copy_from: Annotated[
        str | None,
        typer.Option("--copy-from", help=t("op.opt.copy_from")),
    ] = None,
    interactive: Annotated[bool, typer.Option("--interactive", "-i", help=t("op.opt.interactive"))] = False,
    scope: _ScopeOpt = "task",
    overwrite: Annotated[bool, typer.Option("--overwrite", help=t("op.opt.overwrite"))] = False,
    root: _RootOpt = None,
    as_json: Annotated[bool, typer.Option("--json")] = False,
) -> None:
    r = _resolve_root(root)
    if not pipeline_id.strip():
        typer.secho(
            tr("请指定 --pipeline-id / -p", "Please set --pipeline-id / -p"),
            err=True,
            fg=typer.colors.RED,
        )
    ...
    if sc not in ("task", "domain", "general"):
        raise typer.BadParameter(tr("scope 须为 task|domain|general", "scope must be task|domain|general"))
    ...
    try:
        if from_file is not None:
            ...
            if not typer.confirm(tr("交互式添加算子", "Interactive add operator"), default=True):
                raise typer.Exit()
            in_name = typer.prompt(tr("算子名称", "Operator name"))
            in_desc = typer.prompt(tr("描述", "Description"))
            in_cat = typer.prompt(
                tr("分类", "Category") + f" ({'/'.join(sorted(VALID_CATEGORIES))})",
                default="bridge",
            )
            in_in = typer.prompt(tr("input_keys（逗号分隔）", "input_keys (comma-separated)"), default="records")
            in_out = typer.prompt(tr("output_keys（逗号分隔）", "output_keys (comma-separated)"), default="records")
            in_llm = typer.confirm(tr("requires_llm？", "requires_llm?"), default=False)
            ...
            if template_name not in merged:
                raise ManualOperatorError(
                    tr(f"模板算子不存在: {template_name}", f"Template operator not found: {template_name}")
                )
            if not name:
                raise ManualOperatorError(
                    tr("使用 --copy-from 时请同时提供新算子名称", "Provide new operator name when using --copy-from")
                )
            ...
            if not description.strip():
                typer.secho(
                    tr("请提供 --description，或使用 --from-file / -i", "Provide --description, or use --from-file / -i"),
                    err=True,
                    fg=typer.colors.RED,
                )
                raise typer.Exit(code=2)
...
@operators_app.command("remove", help=t("op.remove.doc"))
def cmd_remove(
    name: Annotated[str, typer.Argument(help=t("op.arg.remove"))],
    pipeline_id: Annotated[str, typer.Option("--pipeline-id", "-p", help=t("opt.pipeline_id"))],
    scope: _ScopeOpt = "task",
    root: _RootOpt = None,
    as_json: Annotated[bool, typer.Option("--json")] = False,
) -> None:
    ...
    if as_json:
        ...
        typer.secho(tr("已删除", "Removed") + f" {out['name']} @ {out.get('path')}", fg=typer.colors.GREEN)
    else:
        typer.secho(out.get("detail", "not found"), fg=typer.colors.YELLOW)


@operators_app.command("show", help=t("op.show.doc"))
def cmd_show(
    name: Annotated[str, typer.Argument(help=t("op.arg.show"))],
    ...
    merged = store.merged_raw()
    if n not in merged:
        typer.secho(tr(f"未找到算子: {n}", f"Operator not found: {n}"), err=True, fg=typer.colors.RED)
