...
from cli.i18n import current_lang, tr
...
def step_label(step_key: str) -> str:
    return (STEP_LABEL_EN if current_lang() == "en" else STEP_LABEL_ZH).get(step_key, step_key)
...
def _format_pilot_dimension_scores_line(detail: dict[str, Any]) -> str | None:
    ...
    for k in PILOT_DIM_ORDER:
        ...
        try:
            ...
            continue
        label = (PILOT_DIM_LABEL_EN if current_lang() == "en" else PILOT_DIM_LABEL_ZH).get(k, k)
...
def print_next_workflow_hint(
    ...
    *,
    ...
    title = ("\n── " + tr("下一步", "Next") + " ──") if verbose else ("\n" + tr("下一步", "Next"))
    typer.secho(title, fg=typer.colors.CYAN, bold=True)
    if done:
        typer.secho("  " + tr("全部步骤已完成。", "All steps completed."), fg=typer.colors.GREEN, bold=(not verbose))
        if verbose:
            typer.echo("  " + tr("查看 token", "Tokens") + f": dataevolver tokens {pipeline_id} --no-events")
        else:
            typer.echo("  " + tr("Token", "Tokens") + f": dataevolver tokens {pipeline_id} --no-events")
        typer.echo("")
        return
    ...
    if evolve and nk == "pipeline_run":
        ...
        if d.get("pilot_feedback_persisted"):
            typer.echo(
                "  "
                + tr("已写入 understanding", "Written to understanding")
        ...
        typer.secho(
            "\n  " + tr("推荐顺序（每步可单独执行）:", "Recommended sequence (each step is manual):"),
        ...
        typer.echo("    3. " + tr("按需", "Optional") + ": evolve-operators → instantiate → trial …")
        typer.secho("\n  " + tr("若仍要全量执行:", "If you still want full run:"), fg=typer.colors.BLUE)
    ...
    if proceed and nk == "pipeline_run":
        if verbose:
            typer.echo(f"  {tr('将执行', 'Will run')}: {nk} — {step_label(nk)}")
            typer.secho(
                f"\n  {tr('下一步命令', 'Next')}: {cli_step_invocation(pipeline_id, nk)}",
...
def _fmt_elapsed(seconds: float | None) -> str:
    ...
    return f" · {m}{tr('分', 'm')}{s:02d}{tr('秒', 's')}"
...
def _print_advance_compact(
    ...
    *,
    ...
    if payload.get("state_unchanged"):
        typer.secho(
            f"✓ {label}{tr('：评估已更新（workflow 状态未改）', ': assessment updated (workflow state unchanged)')}{tim}",
            fg=typer.colors.GREEN,
        )
    ...
    if detail.get("path"):
        typer.secho(f"  {tr('产物', 'Artifact')}: {detail.get('path')}", fg=typer.colors.WHITE)
    ...
    if step == "orchestration" and status == "completed" and "is_valid" in detail:
        ...
        parts = ["DAG " + (tr("通过", "pass") if iv else tr("未通过", "fail"))]
        if tf is True:
            parts.append(tr("任务达标", "task OK"))
        elif tf is False:
            parts.append(tr("任务未达标", "task NOT OK"))
        if rec is True:
            parts.append(tr("建议新算子", "new operators"))
    ...
    if step == "instantiation" and status == "completed":
        ...
        typer.secho(f"  {tr('试跑', 'Try')}: python {entry} --mode pilot", fg=typer.colors.CYAN)

    if step == "trial_run" and status == "completed":
        ok = detail.get("execution_ok")
        typer.echo(f"  {tr('采样执行', 'Sample execution')}: {tr('成功', 'ok') if ok else tr('失败', 'failed')}")
        if detail.get("pilot_overall_score") is not None:
            typer.secho(
                f"  Pilot {tr('总分', 'score')}: {detail.get('pilot_overall_score')} · {detail.get('pilot_recommendation')}",
                fg=typer.colors.CYAN,
            )
            dim_line = _format_pilot_dimension_scores_line(detail)
            if dim_line:
                typer.echo(f"  {tr('各维度', 'Dims')}(0–100): {dim_line}")
        elif detail.get("pilot_judge_skipped"):
            typer.secho(
                f"  Pilot {tr('未评分', 'skipped')} ({detail.get('pilot_judge_skipped')})",
                fg=typer.colors.YELLOW,
            )
        if ok is False:
            typer.secho(
                f"  {tr('重试', 'Retry')}: {cli_step_invocation(pipeline_id, 'trial_run')}",
                fg=typer.colors.YELLOW,
            )

    if step == "pipeline_run" and status == "completed":
        typer.secho(
            "  " + tr("全量结果已落盘（见产物路径）", "Full run results saved (see artifact path)."),
            fg=typer.colors.GREEN,
        )

    if step == "assessment_refresh" and status == "completed" and "is_valid" in detail:
        iv = detail.get("is_valid")
        typer.echo(f"  DAG {tr('评估', 'assessment')}: {tr('通过', 'pass') if iv else tr('未通过', 'fail')}")
    ...
    if next_key == "orchestration":
        out.append(
            tr(
                ...
                f"After editing orchestration JSON, refresh assessment only: dataevolver workflow validate-dag {pipeline_id}",
            )
        )
        ...
        out.append(
            tr(
                ...
                f"Re-assess DAG only: dataevolver workflow validate-dag {pipeline_id}",
            )
        )
        ...
        out.append(
            tr(
                ...
                f"Major DAG change: {_rerun_cmd(pipeline_id, 'orchestration')} → {cli_step_invocation(pipeline_id, 'orchestration')}",
            )
        )
        ...
        out.append(
            tr(
                ...
                f"Redo instantiation/trial: {_rerun_cmd(pipeline_id, 'instantiation')} → {cli_step_invocation(pipeline_id, 'instantiation')}",
            )
        )
        out.append(
            tr(
                f"或一键 pilot（采样+LLM 评估，与 trial 同落盘）：python data/generated_pipelines/{pipeline_id}/run_pipeline.py --mode pilot",
                f"Or one-shot pilot (sample + LLM judge, same artifacts as trial): python data/generated_pipelines/{pipeline_id}/run_pipeline.py --mode pilot",
            )
        )
        ...
        out.append(
            tr(
                ...
                f"Re-run trial: {_rerun_cmd(pipeline_id, 'trial_run')} → {cli_step_invocation(pipeline_id, 'trial_run')}",
            )
        )
        ...
        out.append(
            tr(
                ...
                f"Restart from understanding: {_rerun_cmd(pipeline_id, 'understanding')} → {cli_step_invocation(pipeline_id, 'understanding')}",
            )
        )
    return out
...
def _print_pipeline_assessment_detail(pipeline_id: str, detail: dict[str, Any]) -> None:
    ...
    st = tr("通过", "pass") if iv is True else (tr("未通过", "fail") if iv is False else "—")
    if tf_skip:
        task_w = tr("未评(API 跳过)", "skipped (API)")
    elif tf is True:
        task_w = tr("达标", "OK")
    elif tf is False:
        task_w = tr("未达标", "NOT OK")
    else:
        task_w = "—"
    if rec_ops is True:
        rec_w = tr("是", "yes")
        ...
        rec_w = tr("—(可 validate-dag 刷新)", "— (refresh via validate-dag)")

    fg = typer.colors.GREEN if iv is True else (typer.colors.YELLOW if iv is False else typer.colors.WHITE)
    typer.secho(
        f"\n  {tr('评估', 'Assessment')}: {st}  ·  {tr('结构问题', 'struct issues')} {ic}  ·  {tr('任务', 'task')} {task_w}  ·  {tr('建议新算子', 'new operators')} {rec_w}",
        fg=fg,
        bold=iv is False,
    )
    if reason:
        typer.echo(f"  {tr('摘要', 'Summary')}: {_truncate_line(reason, 200)}")
    if assess_p:
        typer.echo(f"  {tr('完整结果', 'Full')}: {assess_p}")
    if nxt:
        typer.secho("  " + tr("建议下一步", "Suggested next") + ":", fg=typer.colors.MAGENTA)
        for x in nxt:
...
def _print_next_step_blueprint(pipeline_id: str, next_key: str) -> None:
    ...
    if chain:
        typer.secho(f"\n  {tr('链路说明', 'Flow')}: {chain}", fg=typer.colors.WHITE)
    extras = _optional_paths_for_next(pipeline_id, next_key)
    if extras:
        typer.secho("\n  " + tr("可选路径", "Optional") + ":", fg=typer.colors.YELLOW)
        for line in extras:
            typer.echo(f"    · {line}")
...
def print_state_human(pipeline_id: str, st_dict: dict[str, Any], *, verbose: bool = False) -> None:
    ...
    if not verbose:
        typer.secho(f"\n{tr('流水线', 'Pipeline')} {pipeline_id}", fg=typer.colors.CYAN, bold=True)
        typer.echo(f"  {tr('已完成', 'Completed')}: {len(done_list)}")
        ...
        if idx >= len(STEP_ORDER):
            typer.secho("  " + tr("状态: 全部完成", "Status: complete"), fg=typer.colors.GREEN, bold=True)
        else:
            nk = STEP_ORDER[idx]
            typer.secho(
                f"  {tr('下一命令', 'Next')}: {cli_step_invocation(pipeline_id, nk)}",
                fg=typer.colors.WHITE,
                bold=True,
            )
        typer.secho(
            ...
            + tr(
                tr(
                    ...
                    f"(Full status: dataevolver workflow --verbose state {pipeline_id})",
                ),
    ...
    typer.secho("\n── " + tr("Workflow 状态", "Workflow state") + " ──", fg=typer.colors.CYAN, bold=True)
    ...
    typer.echo(
        "  " + tr("覆盖前归档索引", "Archive index") + f": data/artifact_history/{pipeline_id}/index.jsonl"
    )
    typer.echo(f"  {tr('已完成步骤', 'Completed steps')} ({len(done_list)}): ", nl=False)
    if done_list:
        typer.secho(", ".join(done_list), fg=typer.colors.GREEN)
    else:
        typer.secho(tr("(无)", "(none)"), fg=typer.colors.YELLOW)

    if idx >= len(STEP_ORDER):
        typer.secho("  " + tr("状态: 全部步骤已完成。", "Status: all steps completed."), fg=typer.colors.GREEN, bold=True)
        return

    next_key = STEP_ORDER[idx]
    typer.secho(f"  {tr('下一步将执行', 'Next step')}: ", nl=False)
    ...
    typer.secho("\n  " + tr("推荐命令", "Recommended") + ":", fg=typer.colors.CYAN)
...
def print_advance_human(
    ...
    *,
    ...
    typer.secho(f"\n── {tr('本步结果', 'Step result')} ──{tim}", fg=typer.colors.CYAN, bold=True)
    if payload.get("state_unchanged"):
        typer.secho(
            "  "
            + tr(
    ...
    typer.echo(f"  {tr('步骤', 'Step')}: {step} — {step_label(step)}")
    if status == "completed":
        typer.secho(f"  {tr('状态', 'Status')}: {tr('已完成', 'completed')} (completed)", fg=typer.colors.GREEN)
    elif status == "skipped":
        typer.secho(
            "  "
            + tr("状态: 已跳过 (skipped) — 产物已存在", "Status: skipped — artifact already exists"),
            fg=typer.colors.YELLOW,
        )
        if detail.get("detail"):
            typer.echo(f"  {tr('说明', 'Note')}: {detail.get('detail')}")
    else:
        typer.echo(f"  {tr('状态', 'Status')}: {status or 'ok'}")

    if detail.get("path"):
        typer.echo(f"  {tr('产物', 'Artifact')}: {detail.get('path')}")
    ...
    if step == "assessment_refresh" and status == "completed":
        ...
        if detail.get("understanding_feedback_persisted"):
            msg = (
                "  "
                + tr("理解结果: 已同步编排评估（本轮通过）。", "Understanding: assessment synced (passed).")
                if detail.get("is_valid")
                else "  "
                + tr(
    ...
    if step == "operator_evolution":
        ...
        if status == "skipped":
            ...
            names = ", ".join(str(x) for x in added) if added else tr("(无)", "(none)")
            ...
            if soft:
                ...
                typer.secho(
                    "\n  " + tr("算子进化: 硬回退编排，请 orchestrate。", "Operator evolution: hard rewind; please orchestrate."),
                    fg=typer.colors.YELLOW,
                )
    ...
    if step == "instantiation" and status == "completed":
        ...
        if isinstance(cw, list) and cw:
            typer.secho("  " + tr("说明", "Notes") + ":", fg=typer.colors.YELLOW)
            for w in cw[:6]:
                typer.echo(f"    · {w}")
        entry = str(
            detail.get("entry_script") or f"data/generated_pipelines/{pipeline_id}/run_pipeline.py"
        )
        typer.secho("\n  " + tr("一键启动生成管线（推荐）", "Run generated pipeline (recommended)") + ":", fg=typer.colors.GREEN, bold=True)
    ...
    if step == "pipeline_run" and status == "completed":
        typer.secho("\n  " + tr("全量执行已成功落盘。", "Full run saved successfully."), fg=typer.colors.GREEN)

    if step == "experience":
        typer.secho("\n  " + tr("经验快照已落盘。", "Experience snapshot saved."), fg=typer.colors.WHITE)
...
def print_advance_error_human(
    ...
    *,
    ...
    if not verbose:
        typer.secho(f"\n✗ {label} {tr('失败', 'failed')}", fg=typer.colors.RED, bold=True)
        typer.secho(f"  {_truncate_line(message, 220)}", fg=typer.colors.RED)
        typer.secho(f"\n  {tr('重试', 'Retry')}: {cli_step_invocation(pipeline_id, step_key)}", fg=typer.colors.CYAN)
        typer.secho("  " + tr("（原因与分支说明: 加 --verbose）", "(Use --verbose for details.)"), fg=typer.colors.BLUE)
        typer.echo("")
        return

    typer.secho("\n── " + tr("本步失败", "Step failed") + " ──", fg=typer.colors.RED, bold=True)
    typer.echo(f"  {tr('步骤', 'Step')}: {step_key} — {label}")
    typer.secho(f"  {tr('原因', 'Reason')}: {message}", fg=typer.colors.RED)
    typer.secho(f"\n  {tr('可重试', 'Retry')}: {cli_step_invocation(pipeline_id, step_key)}", fg=typer.colors.CYAN)
    hints = _optional_paths_for_next(pipeline_id, step_key)
    if hints:
        typer.secho("\n  " + tr("相关路径", "Related paths") + ":", fg=typer.colors.YELLOW)
        for h in hints:
            typer.echo(f"    · {h}")
    typer.echo("")
...
def print_wrong_step_human(
    ...
    *,
    ...
    typer.secho("\n── " + tr("步骤顺序不符", "Wrong step order") + " ──", fg=typer.colors.RED, bold=True)
    typer.echo(f"  {tr('你请求执行', 'You requested')}: {attempted_step_key} — {step_label(attempted_step_key)}")
    typer.echo(f"  {tr('当前应执行', 'Expected')}: {expected_step_key} — {step_label(expected_step_key)}")
    typer.secho(
        f"\n  {tr('请运行', 'Run')}: {cli_step_invocation(pipeline_id, expected_step_key)}",
        fg=typer.colors.GREEN,
        bold=True,
    )
    typer.secho(
...
def print_tokens_human(summary: dict[str, Any]) -> None:
    typer.secho("\n── " + tr("LLM Token 汇总", "LLM token summary") + " ──", fg=typer.colors.CYAN, bold=True)
    typer.echo(f"  pipeline_id: {summary.get('pipeline_id')}")
    typer.echo(f"  {tr('账本', 'Ledger')}: {summary.get('ledger_path')}")
    typer.echo(f"  {tr('事件条数', 'Events')}: {summary.get('event_count')}")
    typer.secho(
        f"  {tr('合计', 'Total')}: in={summary.get('total_input_tokens')}  out={summary.get('total_output_tokens')}  "
    ...
    if summary.get("total_duration_ms"):
        typer.echo(
            f"  {tr('API 耗时累计(有记录的调用)', 'API duration (recorded calls)')}: {summary.get('total_duration_ms')} ms"
        )
    time_range = summary.get("time_range") or {}
    if time_range.get("first_ts"):
        typer.echo(f"  {tr('时间范围', 'Time range')}: {time_range.get('first_ts')} → {time_range.get('last_ts')}")

    by_step = summary.get("by_workflow_step") or {}
    if by_step:
        typer.secho("\n  " + tr("按 workflow 步骤", "By workflow step") + ":", fg=typer.colors.MAGENTA)
    ...
    if len(by_op) <= 20:
        typer.secho("\n  " + tr("按 operation", "By operation") + ":", fg=typer.colors.MAGENTA)
        for k, v in sorted(
            by_op.items(),
...
def print_rerun_human(result: dict[str, Any]) -> None:
    ...
    typer.secho("\n── " + tr("已重置 workflow", "Workflow reset") + " ──", fg=typer.colors.GREEN, bold=True)
    typer.echo(
        "  "
        + tr(
    ...
    typer.secho("\n  " + tr("已处理产物", "Touched artifacts") + ":", fg=typer.colors.CYAN)
    for a in result.get("artifacts_touched") or []:
        typer.echo(f"    • {a}")
    si = int(result.get("step_index", 0))
    if 0 <= si < len(STEP_ORDER):
        nk = STEP_ORDER[si]
        typer.secho(f"\n  {tr('下一步', 'Next')}: {cli_step_invocation(pid, nk)}", fg=typer.colors.WHITE, bold=True)
    else:
        typer.secho(
            "\n  "
            + tr(
